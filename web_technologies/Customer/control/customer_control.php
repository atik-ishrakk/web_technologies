...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['Submit'])) {
    ...
    if ($isValid) {
        ...
        if ($isValid) {
            ...
            if ($result->num_rows > 0) {
                $stmt = $conn->prepare("UPDATE customers SET fname = ?, lname = ?, phone = ?, email = ?, gender = ?, address = ?, file_path = ? WHERE id = ?");
                $stmt->bind_param("sssssssi", $fname, $lname, $phone, $email, $gender, $address, $uploadedFilePath, $_SESSION['id']);
            } else {
                $stmt = $conn->prepare("INSERT INTO customers (fname, lname, phone, email, gender, address, file_path) VALUES (?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param("sssssss", $fname, $lname, $phone, $email, $gender, $address, $uploadedFilePath);
            }
            ...
            if ($stmt->execute()) {
                ...
                $successMessage = 'Registration successful! <a href="../view/profile.php"> Check your profile here.</a>';
            } else {
